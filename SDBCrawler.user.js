...
(() => {
    ...
    // ── Persistence ────────────────────────────────────────────
    const LZ_KEYS = new Set(['sdb_v2_snapshot', 'itemDatabase']);
    const LZ_VERSION = 1;
    ...
    // ── Hidden rows (non-destructive, persisted) ───────────────
    ...
    // ── Engine: parallel crawl with streaming enrichment ───────
    ...
    // ── Engine: Lebron NC value fallback ────────────────────────
    ...
    async function depositAllInventory() {
        ...
        const deposited = [];
        let total = 0, failed = 0, added = 0, priced = 0;
        try {
            ...
            const stopped = state.stopRequested;
            state.stopRequested = false;
            emit('deposit:done', { done: deposited.length, total, failed, added, priced, stopped });
            emit('data:changed');
        }
    }
...
render();
    ...
    const CSS = `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .brand b { font-weight: 700; background: linear-gradient(90deg, var(--acc), var(--acc-2)); -webkit-background-clip: text; background-clip: text; color: transparent; }
        .dot {
        ...
        .diff-list div { display: flex; gap: 8px; color: var(--text); }
        .diff-list div span:first-child { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .diff-add span:last-child { color: var(--good); }
    ...
    function wireChrome() {
        ...
        ui.btnDiff.addEventListener('click', openDiff);
    ...
    // ── Snapshot diff ─────────────────────────────────────────
    function openDiff() {
        if (!state.items.length) { toast('Nothing to compare: scan or load a snapshot first', true); return; }
        const baseline = Store.get('sdb_baseline', null);
        const snapshot = () => state.items.map(({ id, name, qty }) => ({ id, name, qty }));
        if (!Array.isArray(baseline) || !baseline.length) {
            Store.set('sdb_baseline', snapshot());
            toast(`Baseline saved, ${nf.format(state.items.length)} items. Press Diff again to compare.`);
            return;
        }

        const keyOf = (r) => (r.id != null ? `id:${r.id}` : `name:${String(r.name || '').toLowerCase()}`);
        const base = new Map(baseline.map((r) => [keyOf(r), r]));
        const added = [], changed = [];
        for (const it of state.items) {
            const prev = base.get(it.key);
            if (!prev) added.push({ name: it.name, qty: it.qty });
            else {
                if (prev.qty !== it.qty) changed.push({ name: it.name, delta: it.qty - prev.qty, qty: it.qty });
                base.delete(it.key);
            }
        }
        const removed = [...base.values()].map((r) => ({ name: r.name, qty: r.qty }));

        const esc = escHTML;
        const section = (title, rows, cls, fmt) => `
            <div class="diff-sec">
                <h5>${title} (${nf.format(rows.length)})</h5>
                ${rows.length
                    ? `<div class="diff-list ${cls}">${rows.slice(0, 200)
                        .map((r) => `<div><span>${esc(r.name)}</span><span>${fmt(r)}</span></div>`).join('')}
                       ${rows.length > 200 ? `<div class="diff-none">…and ${nf.format(rows.length - 200)} more</div>` : ''}</div>`
                    : '<div class="diff-none">None</div>'}
            </div>`;

        const { overlay, close } = openModal(`
            <div class="modal-head">Snapshot diff</div>
            <div class="modal-hint">Current list vs. the saved baseline of ${nf.format(baseline.length)} items.</div>
            <div class="tabpane on">
                ${section('Added', added, 'diff-add', (r) => `+${nf.format(r.qty)}`)}
                ${section('Removed', removed, 'diff-del', (r) => `-${nf.format(r.qty)}`)}
                ${section('Quantity changed', changed, '',
                    (r) => `${r.delta > 0 ? '+' : ''}${nf.format(r.delta)} → ${nf.format(r.qty)}`)}
            </div>
            <div class="modal-actions">
                <button class="btn" id="diffRebase">Save current as baseline</button>
                <button class="btn primary" id="diffClose">Close</button>
            </div>`, 'settings', { id: 'diff' });

        overlay.querySelector('#diffClose').addEventListener('click', close);
        overlay.querySelector('#diffRebase').addEventListener('click', () => {
            Store.set('sdb_baseline', snapshot());
            toast(`Baseline updated, ${nf.format(state.items.length)} items`);
            close();
        });
    }
    ...
    function openGuide() {
        const { overlay, close } = openModal(`
                ...
                <div class="guide-body">
                    ...
                    <ul>
                        <li><b>${icon('delta')} Diff</b>: first press saves a baseline; second press shows changes.</li>
    ...
    on('deposit:done', ({ done, failed, added, priced, stopped }) => {
        setBusy(false);
        setProgress(null);
