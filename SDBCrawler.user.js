...
(() => {
    ...
    const COL_DEFS = [
        ...
        { key: 'links',  def: 154,  min: 148, head: 'Links' },
    ];
...
const NEO_CATS = ${JSON.stringify(NEO_CATS)};
...
render();
    ...
    const CSS = `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .c-linkact { display: inline-flex; align-items: center; gap: calc(8px * var(--zoom)); justify-self: start; }
        .c-linkact .act { height: calc(16px * var(--zoom)); }
        .c-linkact .act .ic { width: 1.2em; height: 1.2em; }
        .lnk {
            display: inline-flex; align-items: center; justify-content: center;
            ...
            <section class="panel closed" id="panel">
                ...
                <div class="controls">
                    ...
                    <span class="segjoin solo" id="repriceGroup">
                        <button class="btn" id="btnReprice" title="Refresh all prices (no re-scan)">${icon('refresh')} Reprice</button>
                        <button class="btn gone" id="btnRepriceUnpriced" type="button" title="Price only items ItemDB hasn't valued yet (skips the crawl)">${icon('refresh')} <span id="repriceUnpricedCount"></span></button>
                    </span>
                ...
                <div class="advrow closed" id="advRow">
                    <span class="btnjoin" role="group" aria-label="Include or exclude filters">
                        ...
                        <div class="popmenu closed" id="flagMenu" role="group" aria-label="Flag filters">
                            <div class="tri-list">${TRI_FLAGS.map(([, id, label, title]) => triRow(id, label, title)).join('')}${triRow('hiddenOnly', 'Hidden', 'Show only rows you have hidden')}</div>
                        </div>
                        </span>
                    </span>
    ...
    function mountUI() {
        ...
        if (savedF) {
            ...
            state.triFlags = (tf && typeof tf === 'object')
                ...
                : { inflated: savedF.inflatedOnly ? 1 : 0, canOpen: savedF.canOpen ? 1 : 0 };
            state.filters = {
    ...
    function flagsActive() {
        ...
        return !!(state.hiddenOnly || tf.inflated || tf.canOpen);
    }
    ...
    function wireChrome() {
        ...
        ui.cardColorize.addEventListener('change', (e) => {
            ...
            if (cardColorize && !DETAIL_INTENTS.has(itemdbCfg.intent)) {
                itemdbCfg.intent = 'full';
                saveItemdbCfg();
                const sel = shadowRoot.querySelector('#idbIntent');
                if (sel) sel.value = 'full';
                toast('Colored Cards on · detail set to Full (keeps prices and inflation) · scan to fetch');
            }
            scheduleUpdate();
        });
    ...
    function openSettings(startPane = 'paneGeneral') {
        ...
        const { overlay, close } = openModal(`
            ...
            <div class="tabpane" id="paneItemdb">
                ...
                <label class="modal-row" title="Pricer (default) fetches value only. Card adds colours for Colored Cards. Descriptions &amp; use-types now come from the SDB, so Pricer loses nothing.">Detail level
                ...
                <div class="modal-hint">
                    <b>Full</b>: prices + inflation + colours (needed for Colored Cards)<br>
                    <b>Pricer</b> (default): prices + inflation flags<br>
                    <b>Card</b>: colours only (for Colored Cards), no prices<br>
        ...
        overlay.querySelector('#btnResetLayout').addEventListener('click', () => {
            const ok = window.confirm(
                ...
                + 'the display toggles (Colored Cards, Card Rarity, Link Images, Simplify Numbers) and '
        ...
        const { overlay, close } = openModal(`
                ...
                <div class="guide-body">
                    ...
                    <ul>
                        ...
                        <li><b>${icon('filter')} Filters</b>: two Categories menus (SDB = Neopets categories, ItemDB = ItemDB shop categories) and the Flags (Inflated, Openable) are three-state (click to include, click again to exclude, third click clears). Also Hidden, currency (NC/NP), and rarity/value/qty ranges.</li>
                    ...
                    <ul>
                        <li><b>Full</b>: prices + inflation + colours (needed for Colored Cards).</li>
                        <li><b>Pricer</b> (default): prices + inflation flags.</li>
                        <li><b>Card</b>: colours only (for Colored Cards), no prices.</li>
    ...
    on('reprice:done', ({ stopped, count, unpriced, rateLimited }) => {
        setBusy(false);
        setProgress(null);
        if (rateLimited) { setStatus(`Repriced · ${nf.format(count)} items`); return; }
