...
// @namespace    https://github.com/TamperPanda/SDBCrawler
// @version      1.0.0
// @author       TamperPanda
...
// @homepageURL  https://github.com/TamperPanda/SDBCrawler
// @supportURL   https://github.com/TamperPanda/SDBCrawler/issues
...
(() => {
    ...
    const CFG = {
        ...
        quickstockPerPage: 100,
        depositChunk: 70,
        depositStackCap: 70,
        depositBackfillCap: 40,
    ...
    const slugify = (name) => String(name ?? '').normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase().trim()
        .replace(/\s+/g, '-')
        .replace(/[^\w-]+/g, '')
    ...
    const ALL_STORE_KEYS = [
        ...
        'sdb_ui_position', 'sdb_ui_size', 'sdb_ui_open', 'sdb_ui_max', 'sdb_launcher_corner',
        ...
        'sdb_itemdb_min_delay', 'sdb_itemdb_max_delay', 'sdb_reprice_min_delay', 'sdb_reprice_max_delay',
        'sdb_nc_mode', 'sdb_fetch_itemdb', 'sdb_min_delay', 'sdb_max_delay', 'sdb_debug',
        'sdb_keybinds', 'sdb_custom_keybinds', 'sdb_custom_theme', 'sdb_theme_presets',
        'sdb_card_colorize', 'sdb_card_rarity', 'sdb_link_images', 'sdb_short_values', 'sdb_cache_days',
        'sdb_deposit_rescan', 'sdb_backup_use_lz',
        'sdb_snapshots', 'sdb_snap_trend', 'sdb_history', 'sdb_guide_seen',
    ];
    ...
    const itemdbCfg = {
        ...
        repriceMinDelay: clampInt(Store.get('sdb_reprice_min_delay', CFG.repriceDelay[0]), 0, 120000, CFG.repriceDelay[0]),
        repriceMaxDelay: clampInt(Store.get('sdb_reprice_max_delay', CFG.repriceDelay[1]), 0, 120000, CFG.repriceDelay[1]),
        intent: (i => ITEMDB_INTENTS.includes(i) ? i : 'pricer')(Store.get('sdb_itemdb_intent', 'pricer')),
    ...
    const saveItemdbCfg = () => {
        ...
        Store.set('sdb_reprice_min_delay', itemdbCfg.repriceMinDelay);
        Store.set('sdb_reprice_max_delay', itemdbCfg.repriceMaxDelay);
    ...
    const COL_DEFS = [
        ...
        { key: 'id',     def: 46,   min: 40, head: 'ID' },
        { key: 'move',   def: 82,   min: 56, head: 'Queue' },
        { key: 'links',  def: 112,  min: 104, head: 'Links' },
        { key: 'act',    def: 48,   min: 44,  head: '' },
    ];
    ...
    function colTemplate(cols = COL_DEFS) {
        const tracks = cols.map((c) => {
            const w = colWidths.get(c.key) ?? c.def;
            const min = colMin(c);
            if (w != null) return `calc(${Math.max(min, w)}px * var(--zoom))`;
            const max = Math.max(min, Math.min(Math.ceil(autoMin.get(c.key) || min), ITEM_MAX));
            return `minmax(calc(${min}px * var(--zoom)), calc(${max}px * var(--zoom)))`;
        });
    ...
    function applyColTemplate() {
        ...
        const gaps = COL_DEFS.length * 8 + 24;
    ...
    let editMode = Store.get('sdb_edit_mode', false) === true;
    ...
    const NEO_CATS = ['Album Item', 'Battledome Equipment', 'Books', 'Clothes', 'Neodeck', 'Cures',
        'Favourites', 'Food', 'Gift', 'Grooming', 'Magic Item', 'Petpet', 'Petpetpet', 'Special',
        'Toys', 'TCG', 'Wearables'];
    const NEO_CAT_BIT = new Map(NEO_CATS.map((c, i) => [c, 1 << i]));
    const NEO_FLAG_CATS = [['can_stamp_album', 'Album Item'], ['can_equip', 'Battledome Equipment'],
        ['can_read', 'Books'], ['can_neodeck', 'Neodeck'], ['can_cure', 'Cures'], ['can_feed', 'Food'],
        ['can_groom', 'Grooming'], ['can_petpet', 'Petpet'], ['can_play', 'Toys'], ['can_tcg_album', 'TCG'],
        ['can_closet', 'Wearables']];
    const NEO_TYPE_CATS = new Set(['Clothes', 'Gift', 'Magic Item', 'Petpetpet', 'Special']);
    const computeMetaCats = (raw) => {
        const cats = [];
        for (const [flag, label] of NEO_FLAG_CATS) if (raw[flag]) cats.push(label);
        if (raw.is_favourite) cats.push('Favourites');
        const t = decodeEntities(raw.type_name || '');
        if (NEO_TYPE_CATS.has(t) && !cats.includes(t)) cats.push(t);
        return cats;
    };
    const catsToMask = (labels) => (labels || []).reduce((m, c) => m | (NEO_CAT_BIT.get(c) || 0), 0);
    const maskToCats = (mc) => (mc ? NEO_CATS.filter((c) => mc & NEO_CAT_BIT.get(c)) : []);
    ...
    const hydrate = (row) => {
        ...
        const it = {
            ...
            canRead: !!((row.u || 0) & 2),
            canOpen: !!((row.u || 0) & 8),
            metaCats: maskToCats(row.mc || 0),
            slug: row.slug || null,
            priced: (row.value > 0) || !!row.np,
            blob: '',
        };
        rebuildBlob(it);
        return it;
    ...
    async function apiGetPage(page, refCk, filter = 'np', search = '') {
    ...
    function ingest(rawItems) {
        for (const raw of rawItems) {
            ...
            const it = {
                ...
                description: decodeEntities(raw.obj_desc) || null,
                canEat: !!raw.can_feed, canRead: !!raw.can_read,
                canOpen: false,
                sale: null,
                metaCats: computeMetaCats(raw),
                blob: '',
            };
            rebuildBlob(it);
    ...
    function ingestFields(rawItems) {
        let n = 0;
        for (const raw of rawItems) {
            const id = raw.obj_info_id != null && raw.obj_info_id !== '' ? raw.obj_info_id : null;
            const nameLC = decodeEntities(raw.obj_name).toLowerCase().replace(/\s+/g, ' ').trim();
            let it = (id != null && state.byKey.get(`id:${id}`)) || state.byKey.get(`name:${nameLC}`);
            if (!it) continue;
            if (id != null && it.id == null) it = rekeyItem(it, id);
            if (raw.obj_filename) it.image = imageUrl(raw.obj_filename);
            it.type = decodeEntities(raw.type_name);
            if (raw.obj_rarity != null) { it.rarity = raw.obj_rarity; it.isNC = raw.obj_rarity === 500; }
            const desc = decodeEntities(raw.obj_desc);
            if (desc) it.description = desc;
            it.canEat = !!raw.can_feed;
            it.canRead = !!raw.can_read;
            it.metaCats = computeMetaCats(raw);
            rebuildBlob(it);
            n++;
        }
        if (n) markCatsDirty();
    ...
    const DETAIL_INTENTS = new Set(['full']);
    ...
    function applyMeta(it, meta) {
        ...
        if ('value' in meta && !it.isNC) {
            it.value = normValue(meta.value);
            it.inflated = !!meta.inf;
        }
        if (meta.priced) it.priced = true;
        it.isNC = it.rarity === 500;
        if (meta.slug) it.slug = meta.slug;
        if (!it.image && meta.img) it.image = imageUrl(meta.img);
        if (!it.description && meta.description) it.description = meta.description;
        if (meta.u != null) it.canOpen = !!(meta.u & 8);
        if ('sale' in meta) it.sale = meta.sale ?? null;
    ...
    function itemdbRecord(o, prev, detailed) {
        ...
        if ('saleStatus' in o) {
            const s = o.saleStatus;
            const status = s && typeof s === 'object' ? s.status : s;
            rec.sale = (status === 'ets' || status === 'hts') ? status : null;
        }
        ...
        if (o.slug) {
            const base = slugify(o.name ?? rec.name ?? '');
            if (o.slug !== base) rec.slug = o.slug; else delete rec.slug;
        }
    ...
    async function fetchItemdbByName(names, cache, intent = 'full') {
        const out = new Map();
        if (itemdbUseV1 || !names.length || itemdbAuthExpired) return out;
        try {
            const res = await gmRequest({
                method: 'POST',
                url: CFG.itemdbV2Url,
                headers: { 'Content-Type': 'application/json' },
                data: JSON.stringify({ intent, type: 'name', data: names }),
                timeout: 120000,
            });
            if (res.status === 401) {
                if (!itemdbAuthExpired) {
                    itemdbAuthExpired = true;
                    if (typeof toast === 'function') toast('ItemDB session expired. Open itemdb.com.br, then rescan.', true);
                }
                return out;
            }
            ...
            const detailed = DETAIL_INTENTS.has(intent);
    ...
    async function fetchItemdbChunk(ids, cache, intent = itemdbCfg.intent, fresh = false) {
        if (Date.now() < itemdbCooldownUntil || itemdbAuthExpired) return false;
        ...
        for (let attempt = 1; attempt <= CFG.itemdbRetries; attempt++) {
            ...
            try {
                ...
                const res = await gmRequest({
                    method: 'POST',
                    url: v2 ? (CFG.itemdbV2Url + (fresh ? '?fresh=1' : '')) : CFG.itemdbUrl,
                    headers: { 'Content-Type': 'application/json' },
                    data: v2
                ...
                if (res.status === 401) {
                    if (!itemdbAuthExpired) {
                        itemdbAuthExpired = true;
                        if (typeof toast === 'function') toast('ItemDB session expired. Open itemdb.com.br, then Reprice.', true);
                    }
                    return false;
                }
                if (res.status === 429) {
                    const ra = retryAfterMs(res);
                    const cooldown = ra != null ? ra : 60000;
                    itemdbCooldownUntil = Date.now() + cooldown;
                    let serverMsg = '';
                    try { serverMsg = JSON.parse(res.responseText || '{}').message || ''; } catch {  }
                    const when = cooldown >= 60000
                        ? `about ${Math.max(1, Math.round(cooldown / 60000))} min`
                        : `about ${Math.round(cooldown / 1000)}s`;
                    const note = serverMsg || `ItemDB rate limit. Pricing stopped, try again in ${when}`;
                ...
                if (wantsPrice) for (const id of ids) {
                    const k = String(id), rec = cache.get(k);
                    if (rec) rec.priced = 1; else cache.set(k, { priced: 1 });
                }
                return true;
            } catch (err) {
    ...
    async function runCrawl({ minDelay, maxDelay, useItemdb }) {
        ...
        try {
            ...
            if (resuming) {
                try { ingestFields((await apiGetPage(1, refCk, 'np')).items || []); }
                catch (err) { console.warn('[SDB] resume page-1 favourites refresh failed:', err); }
            }
            ...
            const dispatchEnrichChunk = (chunk) => {
                ...
                enrichChain = enrichChain.then(async () => {
                    try {
                        if (state.stopRequested || Date.now() < itemdbCooldownUntil) return;
            ...
            const pushAndEnqueue = () => {
                ...
                for (; enrichScanned < state.items.length; enrichScanned++) {
                    ...
                    if (it.isNC && !DETAIL_INTENTS.has(itemdbCfg.intent)) continue;
            ...
            emit('data:changed');
            if (!state.stopRequested && useItemdb) state.pricedAt = Date.now();
    ...
    const needsPricing = (it) => it.id != null && !it.isNC && it.rarity !== 500 && !isBlocked(it.id)
        && !(it.value > 0) && !it.priced;
    const unpricedCount = () => state.items.reduce((n, it) => n + (needsPricing(it) ? 1 : 0), 0);

    async function runReprice(onlyUnpriced = false) {
        if (state.crawling || state.withdrawing || state.depositing) return;
        if (Date.now() < itemdbCooldownUntil) {
            const mins = Math.max(1, Math.round((itemdbCooldownUntil - Date.now()) / 60000));
            toast(`ItemDB rate limit still active. Try again in about ${mins} min`, true);
            return;
        }
        const ids = [...new Set(state.items
            .filter((it) => onlyUnpriced ? needsPricing(it)
                : (it.id != null && !it.isNC && it.rarity !== 500 && !isBlocked(it.id)))
            .map((it) => it.id))];
        ...
        itemdbAuthExpired = false;
        emit('reprice:start');
        let done = 0, ok = false, rateLimited = false;
        try {
            ...
            for (let c = 0; c < chunks.length && !state.stopRequested; c++) {
                ...
                const priced = await fetchItemdbChunk(chunks[c], cache, itemdbCfg.intent) !== false;
                applyItemdbCache(cache, chunks[c]);
                persistItemdbCache(cache);
                if (priced) done += chunks[c].length; else enrichFailedIds += chunks[c].length;
                emit('data:changed');
                if (Date.now() < itemdbCooldownUntil) { rateLimited = true; break; }
                if (c < chunks.length - 1) await sleep(randInt(itemdbCfg.repriceMinDelay, itemdbCfg.repriceMaxDelay));
            }
            applyItemdbCache(cache);
            persistItemdbCache(cache, true);
            if (!state.stopRequested) done += await applyLebronFallback(true);
            if (!state.stopRequested) state.pricedAt = Date.now();
            ...
            const stopped = state.stopRequested;
            state.stopRequested = false;
            if (ok) emit('reprice:done', { stopped, count: done, unpriced: enrichFailedIds, rateLimited });
            emit('data:changed');
        }
    }
    ...
    function saveSnapshot(scannedAt = state.scannedAt) {
        const items = state.items.map((it) => {
            const { id, image, name, qty, type, cat, rarity, value, isNC, inflated, sale } = it;
            const u = (it.canEat ? 1 : 0) | (it.canRead ? 2 : 0) | (it.canOpen ? 8 : 0);
            const mc = catsToMask(it.metaCats);
            const row = { id, image, name, qty, type, cat, rarity, value, isNC, inflated };
            if (sale) row.sale = sale;
            if (it.slug) row.slug = it.slug;
            if (it.priced && !(it.value > 0)) row.np = 1;
        ...
        Store.set('sdb_v2_snapshot', { v: 2, scannedAt, pricedAt: state.pricedAt, crawlId: itemsCrawlId, items });
    }

    function loadSnapshot() {
        const snap = Store.get('sdb_v2_snapshot', null);
        ...
        state.pricedAt = snap.pricedAt || snap.scannedAt || null;
            ...
            for (const batch of batches) {
                ...
                if (batchOK) {
                    ...
                    for (const { it, q } of batch) {
                        ...
                        if (moved >= q) continue;
                        if (state.stopRequested) continue;
                        failed += (q - moved);
    ...
    async function depositWithSplit(batch, refCk, out) {
        const itemsIn = (b) => b.reduce((n, s) => n + (s.count || 1), 0);
        try {
        ...
        if (state.stopRequested) return lost;
        await sleep(randInt(500, 1000));
    ...
    async function backfillDepositedFromSDB(rows, refCk) {
        const need = rows.filter((it) => it.id != null && (!it.description || !it.type || !(it.metaCats && it.metaCats.length)));
        if (!need.length || !refCk || !state.scannedAt) return 0;
        const perPage = Math.max(1, CFG.perPage);
        const isFav = (it) => !!(it.metaCats && it.metaCats.includes('Favourites'));
        const pages = new Set();
        for (const nc of [false, true]) {
            const viewNeed = need.filter((it) => !!it.isNC === nc);
            if (!viewNeed.length) continue;
            const view = state.items.filter((it) => !!it.isNC === nc && it.id != null);
            const favCount = view.filter(isFav).length;
            const nonFav = view.filter((it) => !isFav(it)).sort((a, b) => Number(a.id) - Number(b.id));
            const rankOf = new Map(nonFav.map((it, i) => [it.key, i]));
            for (const it of viewNeed) {
                const r = rankOf.get(it.key);
                if (r == null) continue;
                pages.add(`${nc ? 1 : 0}:${Math.floor((favCount + r) / perPage) + 1}`);
            }
        }
        if (!pages.size) return 0;
        if (pages.size > CFG.depositBackfillCap) {
            console.info(`[SDB] deposit backfill skipped: ${pages.size} pages exceed cap ${CFG.depositBackfillCap} — a full scan is cheaper.`);
            return 0;
        }
        emit('status', { text: `Backfilling metadata from ${pages.size} SDB page(s)` });
        let n = 0;
        for (const tag of pages) {
            if (state.stopRequested) break;
            const [ncFlag, page] = tag.split(':');
            try {
                const data = await apiGetPage(Number(page), refCk, ncFlag === '1' ? 'nc' : 'np');
                n += ingestFields(data.items || []);
            } catch (err) { console.warn('[SDB] deposit backfill page', tag, 'failed:', err); }
            await sleep(randInt(scanCfg.minDelay, scanCfg.maxDelay));
        }
        return n;
    }

    async function ingestDeposited(stacks, refCk) {
        ...
        for (const u of stacks) {
            ...
            if (g) { g.count += n; continue; }
            groups.set(key, { key, id: u.id, nameLC, name: u.name, isCash: u.isCash, count: n });
        }
        ...
        if (touched.length) {
            ...
            for (let i = 0; i < ids.length && !state.stopRequested; i += chunkSize) {
                ...
                await fetchItemdbChunk(chunk, cache, itemdbCfg.intent);
            ...
            if (needName.length && !state.stopRequested && !rateLimited) {
                ...
                for (let i = 0; i < names.length && !state.stopRequested; i += chunkSize) {
                    if (Date.now() < itemdbCooldownUntil) { rateLimited = true; break; }
                    const found = await fetchItemdbByName(names.slice(i, i + chunkSize), cache, itemdbCfg.intent);
                    if (!found.size) break;
        ...
        if (touched.length && !state.stopRequested) await backfillDepositedFromSDB(touched, refCk);
    ...
    function groupDepositedForHistory(stacks) {
        const groups = new Map();
        for (const u of stacks) {
            const nameLC = u.name.toLowerCase().replace(/\s+/g, ' ').trim();
            const key = u.id != null ? `id:${u.id}` : `name:${nameLC}`;
            const n = u.count || 1;
            const g = groups.get(key);
            if (g) { g.qty += n; continue; }
            groups.set(key, { key, id: u.id, nameLC, name: u.name, qty: n });
        }
        const byName = new Map();
        for (const it of state.items) if (!byName.has(it.nameLC)) byName.set(it.nameLC, it);
    ...
    async function depositAllInventory() {
        ...
        try {
            ...
            const stacks = items.map((it) => ({
                oii: it.oii, id: it.id, isCash: it.isCash, name: it.name, count: Math.max(1, Number(it.count) || 1),
            }));
            total = stacks.reduce((n, s) => n + s.count, 0);
            const doneItems = () => deposited.reduce((n, s) => n + (s.count || 1), 0);

            const cap = Math.max(1, CFG.depositStackCap);
            const passes = [];
            for (const s of stacks) {
                for (let rem = s.count, p = 0; rem > 0; rem -= cap, p++) {
                    (passes[p] ||= []).push({ oii: s.oii, id: s.id, isCash: s.isCash, name: s.name, count: Math.min(cap, rem) });
                }
            }

            outer:
            for (const pass of passes) {
                for (let i = 0; i < pass.length; i += CFG.depositChunk) {
                    if (state.stopRequested) break outer;
                    const batch = pass.slice(i, i + CFG.depositChunk);
                    emit('deposit:progress', { done: doneItems(), total });
                    failed += await depositWithSplit(batch, refCk, deposited);
                    if (!state.stopRequested) await sleep(randInt(500, 1000));
                }
            }

            if (deposited.length) {
                emit('status', { text: 'Updating snapshot…' });
                ({ added, priced } = await ingestDeposited(deposited, refCk));
            }
            ok = true;
        } catch (err) {
            ...
            const doneCount = deposited.reduce((n, s) => n + (s.count || 1), 0);
            emit('deposit:done', { done: doneCount, total, failed, added, priced, stopped, items: historyItems, attempted });
            emit('data:changed');
        }
    }
    ...
    function passesFilters(it, f) {
        ...
        if (f.neoCatInc || f.neoCatExc) {
            const mc = it.metaCats || [];
            if (f.neoCatExc && mc.some((c) => f.neoCatExc.has(c))) return false;
            if (f.neoCatInc && !mc.some((c) => f.neoCatInc.has(c))) return false;
        }
        ...
        if (f.etsOnly || f.htsOnly) {
            if (!((f.etsOnly && it.sale === 'ets') || (f.htsOnly && it.sale === 'hts'))) return false;
        }
    ...
    function rebuildView() {
        const { query, queryMatch, ncMode, hiddenOnly, triFlags, catFilter, neoCatFilter, filters } = state;
        ...
        let neoCatInc = null, neoCatExc = null;
        for (const c in neoCatFilter) {
            if (neoCatFilter[c] === 1) (neoCatInc || (neoCatInc = new Set())).add(c);
            else if (neoCatFilter[c] === 2) (neoCatExc || (neoCatExc = new Set())).add(c);
        }
        const f = {
            catInc, catExc, catL: catLabel, neoCatInc, neoCatExc, ncMode, triFlags,
            etsOnly: state.etsOnly, htsOnly: state.htsOnly,
    ...
    function pagedNow() {
        return state.cardView || state.pager.mode === 'page';
    }

    function visibleRows() {
        if (!pagedNow()) return state.view;
        const pc = pageCount();
    ...
    function saveFilters() {
        Store.set('sdb_filters', {
            ...
            neoCatFilter: state.neoCatFilter,
            ncMode: state.ncMode,
            hiddenOnly: state.hiddenOnly,
            etsOnly: state.etsOnly,
            htsOnly: state.htsOnly,
    ...
    function updateViewModeUI() {
        const paged = pagedNow();
        const isPage = state.pager.mode === 'page';
        ui.pageBar.classList.toggle('closed', !paged);
        ...
        if (paged && state.pager.page > pageCount()) {
    ...
    function toStandaloneHTML(items) {
        ...
        const cols = COL_DEFS.filter((c) => c.key !== 'move' && c.key !== 'act');
        ...
        const data = items.map((it) => ({
            ...
            metaCats: it.metaCats || [], slug: it.slug || null, priced: it.priced ? 1 : 0,
            nameLC: it.nameLC, blob: it.blob,
        }));
        const jsonEmbed = (o) => JSON.stringify(o).replace(/</g, '\\u003c');
        const json = jsonEmbed(data);
        ...
        if (state.theme === 'custom') {
            ...
            for (const [v] of THEME_VARS) { const hex = normHex(custom[v]); if (hex) rootStyle += ` ${v}: ${hex};`; }
            const font = custom['--font'];
            if (font && /^[a-zA-Z0-9 ,'\-]+$/.test(font)) rootStyle += ` --font: ${font};`;
        }
        ...
        const SHARED_JS = [
            ...
            `const unpricedMark = ${unpricedMark.toString()};`,
            `const VALUE_TEXT = ${JSON.stringify(VALUE_TEXT)};`,
            `const fmtValue = ${fmtValue.toString()};`,
            `const COMPACT_UNITS = ${JSON.stringify(COMPACT_UNITS)};`,
            `const compactNumber = ${compactNumber.toString()};`,
...
.row { display: grid !important; position: relative !important; transform: none !important; height: calc(${CFG.rowH}px * var(--zoom)) !important; }
...
.exp-credit { padding: 10px 14px 14px; font-size: 11px; color: var(--dim); text-align: center; border-top: 1px solid var(--line); }
.exp-credit a { color: inherit; text-decoration: underline; }
</style>
</head>
<body>
<div class="root${linkImages ? ' link-icons' : ''}${themeClass}" id="root" style="${rootStyle}">
  <section class="panel" id="panel">
    <header class="head">
      <div class="brand">${crawlerBadge('sdbcrawlerBadgeExport')}SDB<b>Crawler</b></div>
    ...
    <div class="toolbar">
      ...
      <span class="btnjoin" role="group" aria-label="Include or exclude filters">
        <span class="dropdown" id="neoCatDrop">
          <button class="btn" id="btnNeoCat" type="button" aria-haspopup="true" aria-expanded="false"
                  title="Filter by Neopets category (include / exclude)">SDB <span aria-hidden="true" class="caret">&#x25BE;</span></button>
          <div class="popmenu closed" id="neoCatMenu" role="group" aria-label="Neopets category filters">
            <div class="tri-list" id="neoCatTriList"></div>
            <button class="tri-reset" id="neoCatReset" type="button">Reset categories</button>
          </div>
        </span>
        <span class="dropdown" id="catDrop">
          <button class="btn" id="btnCat" type="button" aria-haspopup="true" aria-expanded="false"
                  title="Filter by ItemDB shop category (include / exclude)">ItemDB <span aria-hidden="true" class="caret">&#x25BE;</span></button>
          <div class="popmenu closed" id="catMenu" role="group" aria-label="Shop category filters">
        ...
        <div class="popmenu closed" id="flagMenu" role="group" aria-label="Flag filters">
          <div class="tri-list">${flagRows}${hiddenRow}</div>
        </div>
        </span>
      </span>
    ...
    <div class="exp-credit">Item data from <a href="https://itemdb.com.br" target="_blank" rel="noopener">ItemDB</a> &#8226; NC values from <a href="https://stylisher.club" target="_blank" rel="noopener">Lebron Values</a></div>
  </section>
</div>
<script>
const DATA = ${json};
const CATS = ${jsonEmbed(cats)};
const NEO_CATS = ${jsonEmbed(NEO_CATS)};
const TRI_CELL = ${jsonEmbed(TRI_CELL)};
const TRI_KEY_BY_ID = ${jsonEmbed(TRI_KEY_BY_ID)};
const nf = new Intl.NumberFormat('en-US');
const collator = new Intl.Collator('en');
...
// Simplify Numbers is baked in at export time (like the theme/zoom): the file has no toggle, so it
// renders value/qty/total the way the panel did. showNum compacts when on; exactTitle keeps the full
// figure on hover, matching the live grid's gridNum/exactTitle pair.
const SHORT = ${shortValues ? 'true' : 'false'};
const showNum = (v) => SHORT ? compactNumber(v) : fmtValue(v);
const exactTitle = (v) => { if (!SHORT) return ''; const exact = fmtValue(v); return compactNumber(v) === exact ? '' : exact; };
const titleAttr = (t) => t ? ' title="' + escHTML(t) + '"' : '';

const st = {
    q: '', qm: null, ncMode: 'all',
    catFilter: {}, neoCatFilter: {}, triFlags: { inflated: 0, canOpen: 0 },
    hiddenOnly: 0, hidden: new Set(),
    sort: { col: 'value', dir: -1 },
    page: 1, pageSize: 100,
};
const triAria = (v) => v === 1 ? 'true' : v === 2 ? 'false' : 'mixed';
const flagsActive = () => st.hiddenOnly || st.triFlags.inflated || st.triFlags.canOpen;
...
function currentF() {
    ...
    let neoCatInc = null, neoCatExc = null;
    for (const c in st.neoCatFilter) {
        if (st.neoCatFilter[c] === 1) (neoCatInc || (neoCatInc = new Set())).add(c);
        else if (st.neoCatFilter[c] === 2) (neoCatExc || (neoCatExc = new Set())).add(c);
    }
    return {
        catInc, catExc, catL: (it) => it.catL, neoCatInc, neoCatExc, ncMode: st.ncMode, triFlags: st.triFlags,
...
function rowHTML(k, i) {
    const d = DATA[k];
    const L = linkUrls(d.name || '', d.slug);
    const noPrice = isUnpriced(d.value);
    const mk = noPrice ? unpricedMark(d) : null;
    const val = noPrice ? mk.t : showNum(d.value);
    const tot = noPrice ? mk.t : showNum(d.total);
    const inf = !noPrice && d.inflated;
    const pvCls = noPrice ? ' ' + mk.c : '';
    const valTitle = noPrice ? mk.title : [exactTitle(d.value), inf ? 'itemdb flags this price as inflated' : ''].filter(Boolean).join(' \\u00b7 ');
    const totTitle = noPrice ? '' : exactTitle(d.total);
    const qtyTitle = exactTitle(d.qty);
    return '<div class="row cols' + (i % 2 ? ' alt' : '') + '" data-k="' + k + '">'
      ...
      + '</div>'
      + '<div class="c-num val' + (inf ? ' inf' : '') + pvCls + '"' + titleAttr(valTitle) + '>' + val + '</div>'
      + '<div class="c-num qty"' + titleAttr(qtyTitle) + '>' + showNum(d.qty) + '</div>'
      + '<div class="c-num tot' + pvCls + '"' + titleAttr(totTitle) + '>' + tot + '</div>'
      ...
      + '<div class="c-linkact">'
      + '<div class="c-links">'
      + '<a class="lnk l-db" target="_blank" rel="noopener" href="' + escHTML(L.db) + '">DB</a>'
      + '<a class="lnk l-jn" target="_blank" rel="noopener" href="' + escHTML(L.jn) + '">JN</a>'
      + '<a class="lnk l-tp" target="_blank" rel="noopener" href="' + escHTML(L.tp) + '">TP</a>'
      + '<a class="lnk l-ah" target="_blank" rel="noopener" href="' + escHTML(L.ah) + '">AH</a>'
      + '</div>'
...
// Neopets metacategory menu: fixed NEO_CATS list, same tri rows as the Shops menu.
function renderNeoCatMenu() {
    const list = $('neoCatTriList');
    list.innerHTML = '';
    for (const c of NEO_CATS) {
        const b = document.createElement('button');
        b.className = 'tri'; b.type = 'button'; b.dataset.cat = c;
        const v = st.neoCatFilter[c] || 0;
        b.dataset.tri = String(v);
        b.setAttribute('role', 'checkbox');
        b.setAttribute('aria-checked', triAria(v));
        b.innerHTML = TRI_CELL;
...
function render() {
    ...
    $('stQty').textContent = showNum(stats.qty);
    $('stValue').textContent = showNum(Math.round(stats.value));
    $('stNC').textContent = nf.format(stats.nc);
    $('btnCat').classList.toggle('on', Object.keys(st.catFilter).length > 0);
    $('btnNeoCat').classList.toggle('on', Object.keys(st.neoCatFilter).length > 0);
...
$('btnNeoCat').addEventListener('click', (e) => { e.stopPropagation(); toggleMenu($('btnNeoCat'), $('neoCatMenu')); });
...
$('neoCatTriList').addEventListener('click', (e) => {
    const t = e.target.closest('.tri'); if (!t) return;
    const c = t.dataset.cat;
    const v = ((st.neoCatFilter[c] || 0) + 1) % 3;
    if (v) st.neoCatFilter[c] = v; else delete st.neoCatFilter[c];
    t.dataset.tri = String(v);
    t.setAttribute('aria-checked', triAria(v));
    viewChanged();
});
$('neoCatReset').addEventListener('click', () => {
    st.neoCatFilter = {};
    for (const t of $('neoCatTriList').querySelectorAll('.tri')) { t.dataset.tri = '0'; t.setAttribute('aria-checked', 'mixed'); }
    viewChanged();
});
...
render();
    ...
    const LINK_ICONS = {
        db: 'data:image/png;base64,',
    ...
    const CSS = `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .root {
            ...
            --row-hover: rgba(124, 108, 255, 0.11);
            --option-bg: #14161e;
        ...
        .root.t-vaporwave { --bg-0: #1a0b30; --bg-1: #2b1454; --bg-2: rgba(255, 106, 213, 0.07); --bg-3: rgba(255, 106, 213, 0.15); --line: rgba(1, 205, 254, 0.24); --line-strong: rgba(1, 205, 254, 0.48); --text: #f6e8ff; --muted: #cfa3ec; --dim: #9271c4; --acc: #ff6ad5; --acc-2: #01cdfe; --acc-grad-2: #b967ff; --good: #05ffa1; --warn: #ffe86b; --bad: #ff5c8a; --input-bg: rgba(0, 0, 0, 0.30); --img-bg: rgba(185, 103, 255, 0.14); --head-bg: rgba(1, 205, 254, 0.06); --row-line: rgba(255, 106, 213, 0.10); --row-alt: rgba(185, 103, 255, 0.05); --row-hover: rgba(255, 106, 213, 0.15); --option-bg: #1a0b30; --toast-bg: rgba(26, 11, 48, 0.96); --backdrop: rgba(12, 4, 28, 0.72); --scroll-thumb: rgba(255, 106, 213, 0.32); --scroll-thumb-hover: rgba(1, 205, 254, 0.55); --panel-blur: blur(16px) saturate(185%); }
        ...
        .root.t-vaporwave .pill.live { color: var(--good); border-color: rgba(5, 255, 161, 0.35); }
        ...
        .segjoin.solo .btn:first-child { border-radius: 8px; }
        ...
        .advrow .field input { width: 84px; height: var(--fh); }
        .advrow .field input.narrow { width: 50px; }
        .advrow .btn { height: var(--fh); }
        .fchip { display: inline-flex; align-items: center; gap: 6px; }
        .btnjoin { display: inline-flex; align-items: stretch; flex: none; height: var(--fh);
            border: 1px solid var(--line-strong); border-radius: 8px;
            background: var(--bg-2); overflow: hidden; }
        .btnjoin .dropdown { display: inline-flex; }
        .btnjoin .btn { height: 100%; border: none; border-radius: 0;
            border-left: 1px solid var(--line-strong); background: transparent; }
        .btnjoin .dropdown:first-child .btn { border-left: none; }
        .btnjoin .btn:hover:not(:disabled) { background: var(--bg-3); filter: none; }
        .btnjoin .btn.on { background: rgba(124, 108, 255, 0.20); color: var(--acc-2); border-color: transparent; }
        .ranges { flex-wrap: wrap; gap: 4px 12px; }
        ...
        .edit-ind {
            ...
            color: var(--acc-2); text-shadow: 0 0 14px rgba(34, 211, 238, 0.45);
        }
        .edit-ind .ic { width: 1.3em; height: 1.3em; }
        .edit-mode .th-act .edit-ind, .edit-mode .cardbar .edit-ind { display: inline-grid; }
        .cardbar .edit-ind { margin-left: auto; margin-right: calc(12px * var(--zoom) + 10px); }
        .btn.gone, .field input.gone, .tri.gone { display: none; }
        ...
        .popmenu .menu-sep { height: 1px; margin: 4px 2px; background: var(--line); }
        ...
        .segset {
            display: inline-flex; align-items: stretch; gap: 0; flex: none; height: calc(var(--fh) - 4px);
            border: 1px solid var(--line-strong); border-radius: 8px;
            background: var(--bg-2); overflow: hidden;
        }
        .segset .seg {
        ...
        .btnreset {
            ...
            height: calc(var(--fh) - 4px); padding: 4px 12px;
            border: 1px solid var(--line-strong); border-radius: 8px;
        ...
        .btnreset:hover { background: var(--bg-3); color: var(--text); }
        .btnreset.on { border-color: var(--acc); color: var(--acc-2); }
        .btnreset.on:hover { background: var(--bg-3); filter: brightness(1.08); }
        .btnreset:focus-visible { outline: 2px solid var(--acc); outline-offset: -2px; }
        ...
        .cols {
            ...
            grid-template-columns: var(--cols-template,
                minmax(calc(90px * var(--zoom)), calc(320px * var(--zoom))) calc(64px * var(--zoom)) calc(58px * var(--zoom))
                calc(110px * var(--zoom)) calc(52px * var(--zoom)) calc(90px * var(--zoom))
                calc(96px * var(--zoom)) calc(126px * var(--zoom)) calc(82px * var(--zoom)) calc(40px * var(--zoom)) minmax(0, 1fr));
        ...
        .colgrip::after {
            ...
            border-radius: 2px; background: var(--line-strong); opacity: 0.38;
        ...
        .th.num { justify-content: flex-end; text-align: right; padding-right: calc(6px * var(--zoom)); }
        ...
        .th.num .arr { flex: none; width: calc(9px * var(--zoom)); text-align: center; }
        ...
        .row .c-num { font-family: var(--mono); font-size: calc(12px * var(--zoom)); font-variant-numeric: tabular-nums; text-align: right; padding-right: calc(6px * var(--zoom)); }
        .row .c-num.id { text-align: center; padding-right: 0; }
        ...
        .r-uncommon   { color: #22c55e; --rar: #22c55e; }
        .r-special    { color: #aa4455; --rar: #aa4455; }
        .r-megarare   { color: #ea580c; --rar: #ea580c; }
        .r-brightred  { color: #ef4444; --rar: #ef4444; }
        .r-retired    { color: #9ca3af; --rar: #9ca3af; }
        .r-artifact   { color: #ef4444; --rar: #ef4444; }
        .r-nc         { color: #d8b4fe; --rar: #d8b4fe; }
        ...
        .c-act {
            ...
            background: var(--bg-2); overflow: hidden;
            opacity: 0; pointer-events: none; transition: opacity 160ms var(--ease-inout);
        }
        .act {
        ...
        .edit-mode .row:hover .c-act, .edit-mode .row.focused .c-act { opacity: 1; pointer-events: auto; }
        .edit-mode .c-act.has-unhide { opacity: 1; pointer-events: auto; }
        ...
        .c-links {
            display: inline-flex; justify-content: flex-start; justify-self: center;
            gap: 0;
            border: 1px solid var(--line-strong); border-radius: calc(5px * var(--zoom));
            background: var(--bg-2); overflow: hidden;
        }
        ...
        .row .c-q { justify-content: flex-end; padding-right: calc(6px * var(--zoom)); }
        .qm, .qp {
        ...
        .val.pv-none, .tot.pv-none, .card .c-nums .v.pv-none { color: var(--good); }
        .val.pv-dim, .tot.pv-dim, .card .c-nums .v.pv-dim { color: var(--dim); }
        ...
        .edit-mode .card:hover .c-act, .edit-mode .card.focused .c-act { opacity: 1; pointer-events: auto; }
        ...
        .modal-card > .toast {
            top: calc(100% + 12px); bottom: auto; max-width: 90%;
            white-space: normal; text-align: center;
        }
        .toast.err { border-color: rgba(248, 113, 113, 0.5); color: var(--bad); }
        .toast.has-action { display: inline-flex; align-items: center; gap: 12px; white-space: nowrap; }
        .toast-action {
            flex: none; padding: 2px 9px; border-radius: 6px;
            border: 1px solid var(--acc); background: transparent; color: var(--acc-2);
            font-size: 11px; font-weight: 600; cursor: var(--cur-pointer); pointer-events: auto;
        }
        .toast-action:hover { background: var(--bg-3); }
        ...
        .modal-hint .ic { display: inline-block; width: 1.05em; height: 1.05em; vertical-align: -0.16em; }
        ...
        .tv-hint { font-size: 11.5px; color: var(--dim); line-height: 1.5; }
        .tv-hint b { color: var(--muted); font-weight: 600; }
        .tv-row input:disabled, #tvFont:disabled { opacity: 0.4; cursor: var(--cur-arrow); }
        .tv-row input[type="color"]:disabled { pointer-events: none; }
        .modal-subhead {
        ...
        #snapSortRow.gone { display: none; }
        .snap-summary { display: flex; align-items: center; gap: 10px; padding: 8px 10px; cursor: var(--cur-pointer); }
        .snap-summary:hover, .snap-summary.open { background: var(--bg-2); }
        .snap-summary:focus-visible { outline: 2px solid var(--acc); outline-offset: -2px; }
        .snap-del {
        ...
        .guide-lead { margin-bottom: 8px !important; }
        .guide-lead a { color: var(--acc); text-decoration: underline; }
        ...
        .guide-credit { margin-top: 14px; padding-top: 10px; border-top: 1px solid var(--line); font-size: 11px; color: var(--dim); text-align: center; }
        .guide-credit a { color: inherit; text-decoration: underline; }
    ...
    const ICONS = {
        ...
        listEdit: '<circle cx="5" cy="7" r="1.3" fill="currentColor" stroke="none"/><circle cx="5" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="5" cy="17" r="1.3" fill="currentColor" stroke="none"/><line x1="9" y1="7" x2="20" y2="7"/><line x1="9" y1="12" x2="20" y2="12"/><line x1="9" y1="17" x2="20" y2="17"/>',
    ...
    const FLAG_ROWS = [
        ['inflated',   'triInflated', 'Inflated', 'Items ItemDB flags as inflated',               'tri'],
        ['etsOnly',    'etsOnly',     'ETS',      'Only items ItemDB marks Easy To Sell',         'two'],
        ['htsOnly',    'htsOnly',     'HTS',      'Only items ItemDB marks Hard To Sell',         'two'],
        ['canOpen',    'triCanOpen',  'Openable', 'Items ItemDB marks as openable (Full detail)', 'tri'],
        ['hiddenOnly', 'hiddenOnly',  'Hidden',   'Show only rows you have hidden',               'two'],
    ];
    const TRI_FLAGS = FLAG_ROWS.filter((r) => r[4] === 'tri');
    const TWO_IDS = new Set(FLAG_ROWS.filter((r) => r[4] === 'two').map((r) => r[1]));
        ...
        <div class="root" id="root">
            ...
            <section class="panel closed" id="panel">
                ...
                <div class="controls">
                    ...
                    <span class="segjoin solo" id="repriceGroup">
                        <button class="btn" id="btnReprice" title="Refresh prices">${icon('refresh')} Reprice</button>
                        <button class="btn gone" id="btnRepriceUnpriced" type="button" title="Price unpriced">${icon('refresh')} <span id="repriceUnpricedCount"></span></button>
                    </span>
                ...
                <div class="toolbar">
                    ...
                    <span class="dropdown">
                        ...
                        <div class="popmenu closed" id="viewMenu" role="group" aria-label="View options">
                            ...
                            <label class="switch" title="Tint card backgrounds with each item&#x2019;s dominant colour, sampled from its image &#xB7; card view only &#xB7; instant, no scan needed">
                            ...
                            <label class="switch" title="Show the four lookup links as the original icons instead of the DB / JN / TP / AH block">
                                <input type="checkbox" id="linkImages"${linkImages ? ' checked' : ''}><span class="track"></span>Classic Links
                            </label>
                            <div class="menu-sep"></div>
                            <label class="switch" title="Reveal the per-row Hide and Remove buttons on hover &#xB7; off keeps the grid read-only">
                                <input type="checkbox" id="editMode"${editMode ? ' checked' : ''}><span class="track"></span>Edit mode
                            </label>
                        </div>
                    </span>
                    <span class="dropdown">
                    ...
                    <span class="dropdown">
                        <button class="btn" id="btnDownload" type="button" aria-haspopup="true" aria-expanded="false" title="Download the current view (filters &amp; sort apply)">${icon('download')} Download <span aria-hidden="true" class="caret">&#x25BE;</span></button>
                        <div class="popmenu closed" id="downloadMenu" role="group" aria-label="Downloads">
                            <button class="btn menu-item" id="exHtml" title="Simplified HTML view, opens in your browser">${icon('download')} HTML</button>
                            <button class="btn menu-item" id="exXls" title="Save as Excel (Item, Value, Qty, Total, Rarity, Category, ID)">${icon('download')} Excel</button>
                            <button class="btn menu-item" id="exCsv" title="Item, Value, Qty, Total, Rarity, Category, ID, NC, Inflation Notice, Image URL">${icon('download')} CSV</button>
                            <button class="btn menu-item" id="exJson" title="Full records for tools &amp; scripts, the same fields as CSV, as JSON">${icon('download')} JSON</button>
                        </div>
                    </span>
                </div>
                <div class="advrow closed" id="advRow">
                    <span class="btnjoin" role="group" aria-label="Include or exclude filters">
                        ...
                        <span class="dropdown" id="flagsDrop">
                        <button class="btn" id="btnFlags" type="button" aria-haspopup="true" aria-expanded="false"
                                title="Inflated, sale status, use-type &amp; hidden filters">Flags <span aria-hidden="true" class="caret">&#x25BE;</span></button>
                        <div class="popmenu closed" id="flagMenu" role="group" aria-label="Flag filters">
                            <div class="tri-list">${FLAG_ROWS.map(([, id, label, title]) => triRow(id, label, title)).join('')}</div>
                        </div>
                        </span>
                    </span>
                    <div class="fchip ranges">
                    ...
                    <button class="btnreset" id="btnFilterClear" title="Reset every filter, range and toggle in this row">Reset</button>
                </div>
                <div class="qbar closed" id="qbar">
                    <span class="info" id="qbarInfo"></span>
                    <button class="qbar-list" id="btnQueueList" title="View &amp; edit the full queue">${icon('listEdit')}</button>
                ...
                <div class="cardbar closed" id="cardBar">
                    ...
                    <span class="edit-ind" aria-hidden="true">${icon('eyeOpen')}</span>
                </div>
                <div class="grid-area" id="gridArea">
                    <div class="gridhead cols" id="gridHead">
                        ...
                        <span class="th ctr">Queue<span class="colgrip" data-col="move" title="Drag to resize &#xB7; double-click to reset"></span></span>
                        <span class="th ctr">Links<span class="colgrip" data-col="links" title="Drag to resize &#xB7; double-click to reset"></span></span>
                        <span class="th ctr th-act" title="Edit mode (toggle in the View menu): reveal per-row Hide and Remove on hover">
                            <span class="edit-ind" aria-hidden="true">${icon('eyeOpen')}</span>
                        </span>
                    </div>
    ...
    function parseColor(str) {
        ...
        if ((m = /^#([0-9a-f]{6})$/i.exec(str))) {
            return [1, 3, 5].map((i) => parseInt(str.slice(i, i + 2), 16)).concat(1);
        }
        if ((m = /^rgba?\(([^)]+)\)$/i.exec(str))) {
    ...
    const rgbHex = ([r, g, b]) => `#${hx2(r)}${hx2(g)}${hx2(b)}`;
    const flattenHex = (c, base) => c[3] >= 1 ? rgbHex(c)
        : rgbHex([0, 1, 2].map((i) => c[i] * c[3] + base[i] * (1 - c[3])));

    function builtinThemeVars(name) {
        const probe = document.createElement('div');
        probe.className = name === 'dark' ? 'root' : `root t-${name}`;
        probe.style.cssText = 'position:absolute; visibility:hidden; pointer-events:none; width:0; height:0';
        shadowRoot.append(probe);
        const cs = getComputedStyle(probe);
        const base = parseColor(cs.getPropertyValue('--bg-0')) || [15, 17, 23, 1];
        const baseRGB = [base[0], base[1], base[2]];
        const out = {};
        for (const [v, , def] of THEME_VARS) {
            const c = parseColor(cs.getPropertyValue(v));
            out[v] = c ? flattenHex(c, baseRGB) : def;
        }
    ...
    function mountUI() {
        ...
        if (savedF) {
            ...
            state.etsOnly = !!savedF.etsOnly;
            state.htsOnly = !!savedF.htsOnly;
            ...
            state.neoCatFilter = {};
            if (savedF.neoCatFilter && typeof savedF.neoCatFilter === 'object') {
                for (const c in savedF.neoCatFilter) { const v = triVal(savedF.neoCatFilter[c]); if (v) state.neoCatFilter[c] = v; }
            }
            const tf = savedF.triFlags;
            state.triFlags = (tf && typeof tf === 'object')
                ? { inflated: triVal(tf.inflated), canOpen: triVal(tf.canOpen) }
                : { inflated: 0, canOpen: 0 };
            state.filters = {
            ...
            if (savedF.advOpen || state.hiddenOnly || state.etsOnly || state.htsOnly || state.ncMode !== 'all'
                || Object.keys(state.catFilter).length
                || Object.keys(state.neoCatFilter).length
        ...
        if (Store.get('sdb_ui_max', false)) {
            const savedPos = Store.get('sdb_ui_position', null);
            const savedW = (size && size.w >= 800) ? size.w : Math.round(window.innerWidth * 0.7);
            const savedH = (size && size.h >= 500) ? size.h : Math.round(window.innerHeight * 0.7);
            preMaxStyle = {
                w: (size && size.w >= 800) ? `${size.w}px` : '',
                h: (size && size.h >= 500) ? `${size.h}px` : '',
                left: savedPos ? `${savedPos.left}px` : '',
                top: savedPos ? `${savedPos.top}px` : '',
                positioned: !!savedPos, pxW: savedW, pxH: savedH,
            };
            maximized = true;
            ui.panel.style.width = ui.panel.style.height = ui.panel.style.left = ui.panel.style.top = '';
            ui.panel.classList.remove('positioned');
            ui.panel.classList.add('maximized');
    ...
    function toggleMaximize() {
        ...
        maximized = !maximized;
        Store.set('sdb_ui_max', maximized);
        maxAnimating = true;
    ...
    function flagsActive() {
        ...
        return !!(state.hiddenOnly || state.etsOnly || state.htsOnly || tf.inflated || tf.canOpen);
    }
    ...
    function syncNeoCatBtn() { if (ui.btnNeoCat) ui.btnNeoCat.classList.toggle('on', Object.keys(state.neoCatFilter).length > 0); }
    function anyFilterActive() {
        return !!(state.query || state.ncMode !== 'all' || flagsActive()
            || Object.keys(state.catFilter).length || Object.keys(state.neoCatFilter).length
            || Object.values(state.filters).some((v) => v != null));
    }
    function syncResetBtn() { if (ui.btnFilterClear) ui.btnFilterClear.classList.toggle('on', anyFilterActive()); }
    ...
    function renderNeoCatMenu() {
        ...
        for (const c of NEO_CATS) {
            ...
            b.type = 'button';
            b.dataset.cat = c;
            const v = state.neoCatFilter[c] || 0;
            b.dataset.tri = String(v);
            b.setAttribute('role', 'checkbox');
            b.setAttribute('aria-checked', triAria(v));
            b.innerHTML = TRI_CELL;
        ...
        ui.neoCatTriList.replaceChildren(frag);
        syncNeoCatBtn();
    }
    const CUSTOM_KEYBIND_ACTIONS = {
        ...
        download:      { label: 'Download menu',       run: () => ui.btnDownload?.click() },
    ...
    function isTypingTarget(e) {
        const el = shadowRoot?.activeElement || e.composedPath?.()[0] || e.target;
    ...
    function wireChrome() {
        ...
        const syncEditMode = () => {
            ui.root.classList.toggle('edit-mode', editMode);
            ui.editMode.checked = editMode;
        };
        ui.editMode.addEventListener('change', (e) => {
            editMode = e.target.checked;
            Store.set('sdb_edit_mode', editMode);
            syncEditMode();
        });
        syncEditMode();
        ...
        anchorMenu(ui.btnNeoCat, ui.neoCatMenu);
        ...
        ui.flagMenu.addEventListener('click', (e) => {
            ...
            if (TWO_IDS.has(t.id)) {
                ...
                t.dataset.tri = state[t.id] ? '1' : '0';
                t.setAttribute('aria-checked', state[t.id] ? 'true' : 'false');
                syncFlagsBtn();
        ...
        ui.neoCatTriList.addEventListener('click', (e) => {
            const t = e.target.closest('.tri'); if (!t) return;
            const c = t.dataset.cat;
            const v = nextTri(state.neoCatFilter[c] || 0);
            if (v) state.neoCatFilter[c] = v; else delete state.neoCatFilter[c];
            t.dataset.tri = String(v);
            t.setAttribute('aria-checked', triAria(v));
            syncNeoCatBtn();
            viewChanged({ swap: false });
        });
        ui.neoCatReset.addEventListener('click', () => {
            state.neoCatFilter = {};
            for (const t of ui.neoCatTriList.querySelectorAll('.tri')) { t.dataset.tri = '0'; t.setAttribute('aria-checked', 'mixed'); }
            syncNeoCatBtn();
            viewChanged({ swap: false });
        });
        ...
        ui.btnFilterClear.addEventListener('click', () => {
            ...
            state.triFlags = { inflated: 0, canOpen: 0 };
        ...
        anchorMenu(ui.btnDownload, ui.downloadMenu, true);
        ui.btnDownload.addEventListener('click', syncDownloadMenu);
        ui.downloadMenu.addEventListener('click', (e) => { if (e.target.closest('button')) closePopMenu(ui.downloadMenu); });

        ui.btnRepriceUnpriced.addEventListener('click', () => runReprice(true));
        ui.exXls.addEventListener('click', () => {
            const rows = exportRows();
        ...
        ui.exHtml.addEventListener('click', () => {
    ...
    function syncDownloadMenu() {
        ...
        for (const b of [ui.exXls, ui.exHtml, ui.exCsv, ui.exJson]) if (b) b.disabled = empty;
    }

    function syncRepriceUnpriced() {
        const n = unpricedCount();
        const show = n > 0 && !(state.crawling || state.withdrawing || state.depositing);
        ui.btnRepriceUnpriced.classList.toggle('gone', !show);
        ui.repriceGroup.classList.toggle('solo', !show);
        if (show) ui.repriceUnpricedCount.textContent = nf.format(n);
    }
    ...
    let toastTimer = 0;
    function toast(text, isErr = false, action = null) {
        const card = activeModal && activeModal.overlay.isConnected
            && activeModal.overlay.querySelector('.modal-card');
        const host = card || ui.panel;
        if (ui.toast.parentElement !== host) host.append(ui.toast);
        ui.toast.textContent = text;
        ui.toast.className = `toast show${isErr ? ' err' : ''}${action ? ' has-action' : ''}`;
        clearTimeout(toastTimer);
        let ttl = isErr ? 6000 : 2400;
        if (action) {
        ...
        toastTimer = setTimeout(() => ui.toast.classList.remove('show'), ttl);
    }
    ...
    function openSettings(startPane = 'paneGeneral') {
        ...
        const { overlay, close } = openModal(`
            ...
            <div class="tabpane" id="paneItemdb">
                ...
                <label class="modal-row" title="Pricer (default) fetches prices only. Full also gets the openable flag; descriptions &amp; use-types otherwise come from the SDB, so Pricer loses nothing.">Detail level
                ...
                <label class="modal-row">Reprice delay (ms)
                    <input id="rpxMin" class="no-spin" type="number" min="0" step="100" value="${itemdbCfg.repriceMinDelay}"
                           title="Pause between Reprice pricing batches &#xB7; larger than ItemDB delay because Reprice has no page crawl to space its requests">&#x2013;
                    <input id="rpxMax" class="no-spin" type="number" min="0" step="100" value="${itemdbCfg.repriceMaxDelay}">
                </label>
                ...
                <div class="modal-hint">
                    <b>Pricer</b> (default): prices + inflation flags<br>
                    <b>Full</b>: prices + inflation + openable flag<br>
                    Raising detail = re-fetches; lowering = keeps existing data<br>
                    <b>${icon('refresh')} Reprice</b> re-fetches at the current detail level<br>
                    <b>Cache Expiry</b>: how long to reuse cached prices
                </div>
                <div class="modal-hint"><b>Scan delay</b> is the pause between SDB pages, <b>ItemDB delay</b> the pause between pricing batches during a scan, <b>Reprice delay</b> the pause between batches during a standalone Reprice (larger, since Reprice has no page crawl to space its requests). Bigger chunks and shorter delays are faster but risk rate-limiting.</div>
            </div>
            ...
            <div class="tabpane" id="paneTheme">
                <label class="modal-row">Theme
                    <select id="tvThemeSel" style="flex:1; width:auto" title="Pick a preset or one of your saved themes"></select>
                </label>
                <div class="tv-hint" id="tvHint"></div>
                <div class="modal-row" id="tvNameRow">Theme name
                    <input id="tvName" type="text" style="flex:1; width:auto" placeholder="My theme"
                           title="Rename this theme, or type a new name to save a copy">
            ...
            <div class="tabpane" id="paneGeneral">
                ...
                <div class="modal-row-block">
                    <div class="modal-hint">Clears prices only, keeps other data. <b>${icon('refresh')} Reprice</b> fills them back in.</div>
                    <div class="modal-actions"><button class="btn" id="btnClearPrices">Clear price data</button></div>
                </div>
                <div class="modal-row-block">
                    <div class="modal-hint">Resets the look (columns, window, zoom, view, toggles, filters/sort) to defaults. Box, prices and settings stay.</div>
                    <div class="modal-actions"><button class="btn" id="btnResetLayout">Reset appearance</button></div>
                </div>
                <div class="danger-box">
                    <div class="modal-head">WARNING</div>
                    <div class="modal-hint">Permanently deletes ALL SDBCrawler data: snapshots, queues, filters, settings, hidden rows, layout and themes.</div>
            ...
            <div class="modal-actions">
                <span class="ma-group gone" id="idbActions">
                    <button class="btn" id="idbSave" title="Save the detail level, chunk size and all three delays">Save</button>
                </span>
                ...
                <span class="ma-group gone" id="themeActions">
                    ...
                    <button class="btn" id="tvReset" title="Discard the custom palette and go back to the Dark preset">Reset theme</button>
                    <button class="btn" id="tvDelete" title="Delete this saved theme">Delete</button>
                    <button class="btn primary" id="tvDup" title="Create your own theme, starting from the current colours">${icon('plus')} Add Theme</button>
                </span>
        ...
        overlay.querySelector('#idbSave').addEventListener('click', () => {
            ...
            itemdbCfg.repriceMinDelay = num('#rpxMin', 0, CFG.repriceDelay[0]);
            itemdbCfg.repriceMaxDelay = Math.max(itemdbCfg.repriceMinDelay, num('#rpxMax', 0, CFG.repriceDelay[1]));
            ...
            toast(`ItemDB: ${itemdbCfg.intent} · ${itemdbCfg.chunk}/chunk · cache ${itemdbCfg.cacheDays}d · `
                + `${itemdbCfg.minDelay}–${itemdbCfg.maxDelay}ms · reprice ${itemdbCfg.repriceMinDelay}–${itemdbCfg.repriceMaxDelay}ms · scan ${scanCfg.minDelay}–${scanCfg.maxDelay}ms`);
        });
        ...
        overlay.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Backspace' || e.key === 'Delete') {
                cur.key = ''; cur.ctrl = cur.shift = cur.alt = false;
        ...
        const livePreview = () => { Store.set('sdb_custom_theme', readPickers()); applyTheme('custom'); };
        ...
        themePane.addEventListener('input', (e) => {
            ...
            if (el.dataset.var) {
                ...
                if (norm) { colorInput(el.dataset.hex).value = norm; livePreview(); }
            }
        });
        themePane.addEventListener('change', (e) => {
            const el = e.target;
            if (el.id === 'tvFont') { livePreview(); return; }
        ...
        const themeSel = themePane.querySelector('#tvThemeSel');
        const nameEl = themePane.querySelector('#tvName');
        const nameRow = themePane.querySelector('#tvNameRow');
        const hintEl = themePane.querySelector('#tvHint');
        const delBtn = overlay.querySelector('#tvDelete');
        const dupBtn = overlay.querySelector('#tvDup');
        const labelOf = (id) => (BUILTIN_THEMES.find(([v]) => v === id) || [id, id])[1];
        const clashesBuiltin = (name) => BUILTIN_THEMES.some(([, l]) => l.toLowerCase() === name.toLowerCase());
        const nameTaken = (name) => clashesBuiltin(name)
            || loadThemePresets().some((p) => p.name.toLowerCase() === name.toLowerCase());
        const freeName = (base) => { let nm = base; for (let n = 2; nameTaken(nm); n++) nm = `${base} ${n}`; return nm; };
        ...
        const applyMode = () => {
            ...
            for (const el of themePane.querySelectorAll('input[data-var], input[data-hex], #tvFont')) el.disabled = builtin;
            nameRow.classList.toggle('gone', builtin);
            delBtn.classList.toggle('gone', mode !== 'saved');
            hintEl.innerHTML = builtin
                ? 'Built-in themes are read-only. Use <b>Add Theme</b> to make your own, or the <b>dice</b> to randomize.'
                : mode === 'saved'
                    ? `Editing <b>${esc(editingName || '')}</b> &#xB7; changes preview live. <b>Save</b> to keep them.`
                    : 'Name your theme and tweak the colours below, then <b>Save</b>.';
        };

        const rebuildThemeSelect = () => {
            const active = activePresetName();
            const presets = loadThemePresets();
            let html = '<optgroup label="Presets">'
                + BUILTIN_THEMES.map(([v, l]) => `<option value="${v}">${esc(l)}</option>`).join('')
                + '</optgroup>';
            if (presets.length) html += '<optgroup label="Your themes">'
                + presets.map((p) => `<option value="preset:${esc(p.name)}">${esc(p.name)}</option>`).join('')
                + '</optgroup>';
            if (state.theme === 'custom' && !active) html += '<option value="custom">Custom (unsaved)</option>';
            themeSel.innerHTML = html;
            themeSel.value = state.theme !== 'custom' ? state.theme : (active ? `preset:${active}` : 'custom');
        };

        const showBuiltin = (id) => { mode = 'builtin'; editingName = null; writePickers(builtinThemeVars(id)); nameEl.value = labelOf(id); applyMode(); };
        const showSaved = (name) => {
            const p = loadThemePresets().find((x) => x.name === name);
            if (!p) return;
            mode = 'saved'; editingName = name; writePickers(p.vars); nameEl.value = name; applyMode();
        };
        const showNew = (vars, name) => { mode = 'new'; editingName = null; writePickers(vars); nameEl.value = name || ''; applyMode(); };

        if (state.theme !== 'custom') showBuiltin(state.theme);
        else { const a = activePresetName(); a ? showSaved(a) : showNew(loadCustomTheme(), ''); }
        rebuildThemeSelect();
        ...
        themeSel.addEventListener('change', () => {
            ...
            if (v.startsWith('preset:')) {
                const name = v.slice(7);
                if (applyThemePreset(name)) { showSaved(name); toast(`Theme "${name}" applied`); }
            } else if (v !== 'custom') {
                applyTheme(v); showBuiltin(v); toast(`Theme "${labelOf(v)}" applied`);
            }
            rebuildThemeSelect();
        });
        ...
        themePane.querySelector('#tvSave').addEventListener('click', () => {
            ...
            if (clashesBuiltin(name)) { toast(`"${name}" is a preset theme name. Pick another.`, true); nameEl.focus(); return; }
            const presets = loadThemePresets();
            const at = presets.findIndex((p) => p.name.toLowerCase() === name.toLowerCase());
            const editingAt = editingName ? presets.findIndex((p) => p.name === editingName) : -1;
            if (at >= 0 && at !== editingAt) { toast(`A saved theme called "${presets[at].name}" already exists.`, true); nameEl.focus(); return; }
            const vars = readPickers();
            const verb = editingAt >= 0 ? 'updated' : 'saved';
            if (editingAt >= 0) presets[editingAt] = { name, vars };
            else presets.push({ name, vars });
            saveThemePresets(presets);
            mode = 'saved'; editingName = name;
            applyThemePreset(name);
            applyMode();
            rebuildThemeSelect();
            toast(`Theme "${name}" ${verb}`);
        });

        dupBtn.addEventListener('click', () => {
            const base = mode === 'builtin' ? builtinThemeVars(themeSel.value) : readPickers();
            showNew(base, freeName('My theme'));
            previewCustom(readPickers());
            rebuildThemeSelect();
            nameEl.focus(); nameEl.select();
            toast('New theme ready. Name it, tweak the colours, then Save.');
        });

        delBtn.addEventListener('click', () => {
            if (mode !== 'saved' || !editingName) return;
            const presets = loadThemePresets();
            const at = presets.findIndex((p) => p.name === editingName);
            if (at < 0) return;
            const [gone] = presets.splice(at, 1);
            saveThemePresets(presets);
            applyTheme('dark'); showBuiltin('dark');
            rebuildThemeDropdown(); rebuildThemeSelect();
        ...
        overlay.querySelector('#tvRandom').addEventListener('click', () => {
            ...
            const reroll = mode === 'new';
            showNew(next, reroll ? nameEl.value : freeName('My theme'));
            previewCustom(readPickers());
            rebuildThemeSelect();
            if (!reroll) { nameEl.focus(); nameEl.select(); }
        });
        ...
        overlay.querySelector('#btnResetLayout').addEventListener('click', () => {
            const ok = window.confirm(
                'Reset to a fresh-install look?\n\n'
                + 'Column widths, window size/position, launcher spot, grid zoom, the rows/cards view, '
                + 'the display toggles (Colored Cards, Card Rarity, Classic Links, Simplify Numbers) and '
                + 'your filters/sort all go back to default. Your SDB items, prices, snapshots, history '
                + 'and settings (keybinds, theme, ItemDB options) are kept.\n\n'
                + 'The page reloads to apply.');
            if (!ok) return;
            colWidths.clear();
            for (const key of [
                'sdb_col_widths', 'sdb_ui_size', 'sdb_ui_position', 'sdb_launcher_corner', 'sdb_ui_max',
                'sdb_grid_zoom', 'sdb_card_view', 'sdb_pager', 'sdb_filters',
                'sdb_card_colorize', 'sdb_card_rarity', 'sdb_link_images', 'sdb_short_values', 'sdb_nc_mode',
            ]) Store.del(key);
            location.reload();
        });
    ...
    function openSnapshots() {
        ...
        let trend = loadTrend();
        if (!state.items.length && !ring.length && !trend.length) { toast('Nothing to compare: scan or load a snapshot first', true); return; }

        const cache = new Map(Object.entries(Store.get('itemDatabase', {})));

        const { overlay, close } = openModal(`
            <div class="modal-head">Snapshots</div>
            <div class="modal-hint">Total value over time, newest first. Click an entry to compare the current box against it.</div>
            <label class="modal-row" id="snapSortRow">Sort changes by
                <select id="snapSort"><option value="name">Name</option><option value="mover" selected>Value</option></select>
            </label>
            <div class="rv-scroll" id="snapList"></div>
            <div class="modal-actions">
        ...
        const sortRow = overlay.querySelector('#snapSortRow');
        ...
        const fillDetail = (detail, ts) => {
            const entry = ring.find((e) => e.ts === ts);
            detail.innerHTML = entry
                ? renderSnapshotDiff(entry, cache, sortSel.value)
                : '<div class="diff-none">Item-level detail for this point is no longer stored.</div>';
        };
        ...
        const render = () => {
            ...
            openDetails.clear();
            if (!trend.length) {
                sortRow.classList.add('gone');
                const empty = document.createElement('div');
                empty.className = 'diff-none';
                empty.textContent = 'No snapshots yet. Click "Snapshot now" to save your first.';
                listEl.append(empty);
                return;
            }
            sortRow.classList.remove('gone');
            for (let i = trend.length - 1; i >= 0; i--) {
                const p = trend[i];
                const prevVal = i > 0 ? trend[i - 1].value : null;
                const delta = prevVal == null ? '' : ` (${fmtDelta(p.value - prevVal)})`;

                const rowEl = document.createElement('div');
                rowEl.className = 'hist-entry';

                const summary = document.createElement('div');
                summary.className = 'snap-summary';
                summary.setAttribute('role', 'button');
                summary.tabIndex = 0;

                const when = document.createElement('span');
                when.className = 'trend-when';
                when.textContent = fmtWhen(p.ts) + (p.label ? ` · ${p.label}` : '');
                const val = document.createElement('span');
                val.className = 'trend-val';
                val.textContent = `${nf.format(p.value)} NP${delta}`;
                ...
                summary.append(when, val, del);

                const detail = document.createElement('div');
                detail.className = 'hist-detail gone';
                let built = false;
                const toggle = () => {
                    if (!built) { built = true; fillDetail(detail, p.ts); }
                    const open = detail.classList.toggle('gone') === false;
                    summary.classList.toggle('open', open);
                    if (open) openDetails.set(p.ts, detail); else openDetails.delete(p.ts);
                };
                summary.addEventListener('click', (ev) => { if (!ev.target.closest('.snap-del')) toggle(); });
                summary.addEventListener('keydown', (ev) => {
                    if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); toggle(); }
                });
                del.addEventListener('click', (ev) => {
                    ev.stopPropagation();
                    ({ trend, ring } = deleteTrendAt(p.ts));
                    render();
                    toast('Snapshot removed');
                });

                rowEl.append(summary, detail);
                listEl.append(rowEl);
            }
        };
        render();
        ...
        sortSel.addEventListener('change', () => {
            for (const [ts, detail] of openDetails) fillDetail(detail, ts);
        });
    ...
    function openGuide() {
        const { overlay, close } = openModal(`
                <div class="modal-head">Quickstart Guide</div>
                <div class="guide-body">
                    <p class="guide-lead"> See the full guide <a href="https://github.com/TamperPanda/SDBCrawler/blob/main/README.md" target="_blank" rel="noopener">here</a>.</p>
                    <ul>
                        <li>To log your Safety Deposit Box, click the launcher to open it and press <b>${icon('play')} Start scan</b>.</li>
                        <li>Click an item image to copy its name; hover most buttons for a quick description.</li>
                        <li>Hotkeys can be assigned in <b>Settings${icon('play')} Keybinds.</b></li>
                        <li>Pick a preset theme from the selector at the top right, or build your own in <b>Settings${icon('play')} Theme.</b> Clicking the ${icon('dice')} applies a randomized theme.</li>
                        <li>Switch between Rows/Cards and Scroll/Paged modes in the <b>View</b> dropdown (cards are always paged; set the page size, up to 1000, at the bottom right).</li>
                        <li>Drag the header to move the panel, its bottom-right corner to resize, or the collapsed launcher button to reposition it.</li>
                        <li><b>Zoom</b> scales the grid. Try adjusting zoom when changing panel size if you find it looks oddly spaced after resizing. Columns can also be resized by dragging their headers.</li>
                        <li>Prices are fetched with your itemdb browser session: visiting the site gives you 24 hours, logging in lasts 14 days. If prices stop filling in, reopen itemdb.com.br then rescan or <b>${icon('refresh')} Reprice</b>.</li>
                        <li><b>${icon('box')} Deposit</b> Quickstocks your whole inventory into your SDB, fetches itemdb data for the new items, and adds them to your log.</li>
                        <li>Type in the search box to find items by name, category or ID. Click <b>${icon('filter')} Filters</b> to expand the filter menu.</li>
                        <li>In Rows view, click a column heading to sort by it, and again to reverse. Cards have a <b>Sort by</b> dropdown.</li>
                        <li>The <b>+</b> in the Queue column (or a card&#x2019;s bottom-right) queues an item. Shift+Click queues the stack. You can move items to Inventory, Shop or Gallery.</li>
                        <li>The <b>${icon('eyeOpen')}</b> eye hides a row; the <b>${icon('x')}</b> removes it from the log.</li>
                    </ul>
                    <div class="guide-credit">Item data from <a href="https://itemdb.com.br" target="_blank" rel="noopener">itemdb</a> &#8226; NC values from <a href="https://stylisher.club" target="_blank" rel="noopener">Lebron Values</a><br>SDBCrawler by <a href="https://github.com/TamperPanda/SDBCrawler" target="_blank" rel="noopener">TamperPanda</a></div>
                </div>
                <div class="modal-actions">
    ...
    function openPasteList() {
        ...
        const { overlay, close } = openModal(`
                ...
                <div class="modal-hint">One item per line, case-insensitive. Add <b>, N</b> or a tab after a name to set
    ...
    let swCanvas, swCtx;
    function computeSwatch(img) {
        try {
            const w = img.naturalWidth, h = img.naturalHeight;
            if (!w || !h) return null;
            if (!swCanvas) { swCanvas = document.createElement('canvas'); swCtx = swCanvas.getContext('2d', { willReadFrequently: true }); }
            swCanvas.width = w; swCanvas.height = h;
            swCtx.clearRect(0, 0, w, h);
            swCtx.drawImage(img, 0, 0);
            const d = swCtx.getImageData(0, 0, w, h).data;
            let vr = 0, vg = 0, vb = 0, vw = 0, fr = 0, fg = 0, fb = 0, fn = 0;
            for (let i = 0; i < d.length; i += 4) {
                if (d[i + 3] < 128) continue;
                const r = d[i], g = d[i + 1], b = d[i + 2];
                if (!(r > 245 && g > 245 && b > 245)) { fr += r; fg += g; fb += b; fn++; }
                const mx = Math.max(r, g, b), mn = Math.min(r, g, b);
                const s = mx ? (mx - mn) / mx : 0, wt = s * s * (mx / 255);
                vr += r * wt; vg += g * wt; vb += b * wt; vw += wt;
            }
            const hex = (r, g, b) => '#' + [r, g, b].map((x) => Math.round(x).toString(16).padStart(2, '0')).join('');
            if (vw > 1) return hex(vr / vw, vg / vw, vb / vw);
            if (fn) return hex(fr / fn, fg / fn, fb / fn);
            return null;
        } catch { return null; }
    }
    function tintCard(el, it) {
        const hex = (cardColorize && !(state.queue.get(it.key) > 0)) ? swatchCache.get(it.image) : null;
        el.style.background = hex ? `${hex}22` : '';
        el.style.borderColor = hex ? `${hex}66` : '';
    }
    function ensureSwatch(el, it) {
        const url = it.image;
        if (!url || swatchCache.has(url)) return;
        const img = el._c.img;
        const read = () => {
            if (swatchCache.has(url) || img.dataset.src !== url || !img.complete || !img.naturalWidth) return;
            swatchCache.set(url, computeSwatch(img));
            if (el._key === it.key) tintCard(el, it);
        };
    ...
    function makeRow() {
        ...
        el.innerHTML = `
            <div class="c-item"><img loading="lazy" decoding="async" crossorigin="anonymous" alt="" title="Click to copy item name"><span class="name"></span></div>
    ...
    const VALUE_TEXT = { 'permanent buyable': 'NC Mall' };
    ...
    const unpricedMark = (it) =>
        it.isNC    ? { t: '?', c: 'pv-dim',  title: 'No price' }
      : it.priced  ? { t: '?', c: 'pv-none', title: 'ItemDB has no price for this item' }
      :              { t: '–', c: 'pv-dim',  title: 'Not priced yet' };
    ...
    const linkUrls = (name, slug) => {
        const plus = encodeURIComponent(name).replace(/%20/g, '+');
        return {
            db: `https://itemdb.com.br/item/${slug || slugify(name)}`,
    ...
    function assignRow(el, it, index) {
        ...
        const mk = noPrice ? unpricedMark(it) : null;
        r.val.textContent = noPrice ? mk.t : gridNum(it.value);
        r.val.classList.toggle('inf', !noPrice && !!it.inflated);
        r.val.classList.remove('pv-none', 'pv-dim');
        if (noPrice) r.val.classList.add(mk.c);
        r.val.title = noPrice ? mk.title
        ...
        r.tot.textContent = noPrice ? mk.t : gridNum(total);
        r.tot.classList.remove('pv-none', 'pv-dim');
        if (noPrice) r.tot.classList.add(mk.c);
        r.tot.title = noPrice ? '' : exactTitle(total);

        const L = linkUrls(it.name, it.slug);
        r.ldb.href = L.db;
        r.ljn.href = L.jn;
        r.ltp.href = L.tp;
        r.lah.href = L.ah;
        ...
        el.innerHTML = `
            <div class="c-top">
                <span class="thumbwrap">
                    <img loading="lazy" decoding="async" crossorigin="anonymous" alt="" title="Click to copy item name">
                    <span class="rar"></span>
                </span>
    ...
    function assignCard(el, it) {
        ...
        if (cardColorize) ensureSwatch(el, it);
        r.name.textContent = it.name;
        r.rar.textContent = rarityLabel(it.rarity);
        ...
        r.nums[1].textContent = noPrice ? mk.t : compactNumber(it.value);
        r.nums[1].classList.toggle('inf', !noPrice && !!it.inflated);
        r.nums[1].classList.remove('pv-none', 'pv-dim');
        if (noPrice) r.nums[1].classList.add(mk.c);
        r.nums[1].title = noPrice ? mk.title
            : [exact(it.value), it.inflated ? 'itemdb flags this price as inflated' : ''].filter(Boolean).join(' · ');
        r.nums[2].textContent = noPrice ? mk.t : compactNumber(total);
        r.nums[2].classList.remove('pv-none', 'pv-dim');
        if (noPrice) r.nums[2].classList.add(mk.c);
    ...
    let catSignature = '';
    function syncOpenableRow() {
        const has = state.items.some((it) => it.canOpen);
        if (ui.triCanOpen) ui.triCanOpen.classList.toggle('gone', !has);
        if (!has && state.triFlags.canOpen) {
            state.triFlags.canOpen = 0;
            if (ui.triCanOpen) { ui.triCanOpen.dataset.tri = '0'; ui.triCanOpen.setAttribute('aria-checked', 'mixed'); }
            syncFlagsBtn();
            viewChanged({ swap: false });
        }
    }
    ...
    function renderMeta() {
        ...
        const absTime = (ts) => ts ? new Date(ts).toLocaleString() : '—';
        if (state.items.length) {
            ...
            if (state.pricedAt) {
                const stale = Date.now() - state.pricedAt > itemdbCfg.cacheDays * 86400000;
                const span = document.createElement('span');
                if (stale) span.className = 'stale';
                span.textContent = `priced ${timeAgo(state.pricedAt)}`;
                ui.footInfo.append(' · ', span);
                ui.footInfo.title = `Scanned ${absTime(state.scannedAt)} · Priced ${absTime(state.pricedAt)}`;
            } else {
                ui.footInfo.title = state.scannedAt ? `Scanned ${absTime(state.scannedAt)}` : '';
            }
        } else {
            ui.footInfo.textContent = 'No data yet';
            ui.footInfo.title = '';
        }
        syncRepriceUnpriced();
    ...
    on('reprice:done', ({ stopped, count, unpriced, rateLimited }) => {
        setBusy(false);
        setProgress(null);
        if (rateLimited) { setStatus(`Rate limited · ${nf.format(count)} priced`); return; }
    ...
    on('withdraw:done', ({ done, failed, stopped, action, refused = [], notInBox = 0, items = [] }) => {
        ...
        toast(`Moved ${nf.format(done)} unit${done === 1 ? '' : 's'} to your ${where}${failTxt}${refTxt}${skipTxt}`,
            failed > 0 || notInBox > 0, { label: 'View activity', onClick: openHistory });
    });
    on('deposit:start', () => {
        setBusy(true);
    ...
    on('deposit:done', ({ done, failed, added, priced, stopped, items = [], attempted = false }) => {
        ...
        toast(`${stopped ? 'Deposit stopped, ' : 'Deposited '}${nf.format(done)} unit${done === 1 ? '' : 's'}${failTxt}`
            ...
            attempted ? { label: 'View activity', onClick: openHistory } : null);
    });
    let colMinsTimer = 0;
    ...
    if (!Store.get('sdb_guide_seen', false)) { Store.set('sdb_guide_seen', true); openGuide(); }
